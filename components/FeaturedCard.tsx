...
export default function FeaturedCard({ property }:{ property: Property }){
    ...
    return(
        <TouchableOpacity className="w-80 mr-2 rounded-3xl overflow-hidden bg-white-500"
        style={{
            shadowColor: "#000",
        ...
        <View className="absolute top-3 left-3 bg-white/90 px-3 py-1 rounded-full">
            <Text className="text-xs font-semibold text-blue-600">{property.type}</Text>
        </View>
        {property.is_sold && (
        ...
        <View className="p-4">
            <Text className="text-base font-bold text-gray-800 mb-1">{property.title}</Text>
